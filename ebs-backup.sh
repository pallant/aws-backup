...
snapshot_volumes() {
	for volume_id in $volume_list; do
		...
		snapshot_description=$server_name"-$(hostname)-$device_name-backup-$(date +%F_%T)"
		...
		aws ec2 create-tags --region $region --resource $snapshot_id --tags Key=CreatedBy,Value=AutomatedBackup Key=Owner,Value=TFP
	done
}
